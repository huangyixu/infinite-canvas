...
- 所有页面的顶部操作区均不再展示版本号和 GitHub 图标，首页主操作区不再展示“提示词仓库”按钮。
- 登录页不再展示“支持账号密码和 Linux.do 登录。”说明文案。
- 顶部导航栏的头像、配置按钮和主题切换按钮在悬停时显示手形光标。
- 待人工确认：首页、登录页、业务页、画布页和管理页在桌面端、移动端以及浅色、深色主题下均不再出现版本号和 GitHub 图标，顶部导航交互元素均有正确的悬停光标反馈。
